...
int luaV_lessthan (lua_State *L, const TObject *l, const TObject *r) {
  int res;
  if (ttype(l) != ttype(r)) {
    /* LuaPlus: mixed-type comparisons return false instead of error */
    return 0;
  }
 ...
 retentry:  /* entry point when returning to old functions */
  ...
  for (;;) {
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_LEN: {  /* LuaPlus: # length operator */
        const TObject *rb = RB(i);
        if (ttistable(rb)) {
          /* Find last non-nil array element (like Lua 5.1's # operator) */
          Table *h = hvalue(rb);
          int j = h->sizearray;
          while (j > 0 && ttisnil(&h->array[j - 1])) j--;
          setnvalue(ra, (lua_Number)j);
        }
        else if (ttisstring(rb)) {
          setnvalue(ra, (lua_Number)tsvalue(rb)->tsv.len);
        }
        else {
