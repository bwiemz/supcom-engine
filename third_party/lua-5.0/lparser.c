...
typedef struct BlockCnt {
  ...
  int continuelist;  /* list of jumps to loop condition (LuaPlus extension) */
...
static UnOpr getunopr (int op) {
  switch (op) {
    ...
    case '#': return OPR_LEN;  /* LuaPlus: length operator */
...
static void whilestat (LexState *ls, int line) {
  ...
  block(ls);
  luaK_patchtohere(fs, bl.continuelist);  /* continue jumps to condition */
...
static void forbody (LexState *ls, int base, int line, int nvars, int isnum) {
  ...
  block(ls);
  luaK_patchtohere(fs, bl.continuelist);  /* continue jumps to loop step */
...
/* LuaPlus extension: continue statement */
...
static int statement (LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    case TK_CONTINUE: {  /* stat -> continuestat (LuaPlus extension) */
      continuestat(ls);
      return 1;  /* must be last statement */
    }
    default: {
