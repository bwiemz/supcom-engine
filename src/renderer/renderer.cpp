...
bool Renderer::init(u32 width, u32 height, const std::string& title) {
    ...
    // Bloom resources first — scene_render_pass_ (HDR) is needed by scene pipelines
    create_bloom_resources();

    // Pipelines (scene pipelines built against scene_render_pass_ for HDR bloom)
    create_pipelines();
    ...
    // Particle renderer (emitter-driven billboard particles, renders to HDR scene)
...
void Renderer::create_bloom_resources() {
    ...
    // Scene color uses HDR format so bloom can extract overbright values.
    // Scene pipelines are built against scene_render_pass_ (HDR) and the
    // composite pass tone-maps back to swapchain format.
    {
        VkImageCreateInfo img_ci{};
        img_ci.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        img_ci.imageType = VK_IMAGE_TYPE_2D;
    ...
    // Scene render pass (color + depth, HDR format for bloom overbright extraction)
    {
        VkAttachmentDescription color_att{};
    ...
    // Bloom render pass (single color, no depth)
    {
        ...
        // Incoming: wait for previous pass writes before reading
        VkSubpassDependency bloom_deps[2]{};
        ...
        bloom_deps[0].dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
        bloom_deps[0].srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
        bloom_deps[0].dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
        // Outgoing: finalLayout transition visible to subsequent fragment reads
        ...
        rp_ci.pDependencies = bloom_deps;
...
void Renderer::render(sim::SimState& sim, lua_State* L,
    ...
    // Always render scene to HDR offscreen (scene_render_pass_) for bloom compatibility.
    // The composite pass transfers to swapchain with or without bloom.
    ...
    if (particle_renderer_.draw_count() > 0) {
        ...
        auto view = math::look_at(eye_x, eye_y, eye_z,
                            camera_.target_x(), 0.0f, camera_.target_z(),
                            0.0f, 1.0f, 0.0f);
        // Column-major: right = row 0, up = row 1
    ...
    // ==================== COMPOSITE / BLOOM PASSES ====================
    // Scene always renders to HDR offscreen. End scene pass, optionally run
    // bloom bright extract + blur, then composite to swapchain.
    vkCmdEndRenderPass(cmd_buf_[fi]);
    ...
    // Begin swapchain render pass for composite + UI
    {
        ...
        // Composite fullscreen triangle — blend HDR scene (+bloom if active) onto swapchain
