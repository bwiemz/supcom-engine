...
| M208 | Save/load | Design: `docs/plans/2026-09-25-m208-save-load-design.md`. A save is the game's recording (setup and commands). Loading replays it headless to the saved tick, checked against its checksum trail, then hands over to the player: correct by construction, since replays play back identically. Retail's `InternalSaveGame` and `LoadSavedGame` are wired to it (M208a). M208b measures load times. M208c adds state snapshots (Pluto for Lua 5.0 plus C++ serializers) only if loads are too slow. |
