...
| D | In progress: M195 (ordered iteration), M196 (one sim RNG, checksum traces) and M197 (floating-point policy) are done. So are M198a (every player input reaches the sim as an in-tick command), M199a (replays record, play back headlessly and check every tick) and M199c (a Windows build and a Linux build play a real game identically). Still to do: M198b (consensus on a dropped peer) and M199b (windowed playback). |
| E–I | Not started. |
