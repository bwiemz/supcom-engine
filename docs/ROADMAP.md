...
| D | In progress: M195 (ordered iteration), M196 (one sim RNG, checksum traces) and M197 (floating-point policy) are done. M198a is done: SimCallbacks and the orders panel's unit settings run in-tick on every peer. M198b (consensus on a dropped peer) and M199 (replays) remain. |
| E–I | Not started. |
