...
| M191 | Sim/User split | Design: `docs/plans/2026-09-24-m191-sim-user-split-design.md`. **Step 1 ✅** The library cycle is broken: the 34 UI bindings that use the renderer (the camera, `UIWorldView` projection, the selection, `IssueCommand`, ...) moved into `osc_lua_user`, so the sim's Lua library no longer links the renderer. `arch.link_layers` fails on a link cycle, or on the sim, its Lua library or the blueprints depending on a layer above them. **Step 2 ✅** `moho_bindings.cpp` (16k lines) is split by class into `src/lua/bindings/{sim,ui}/`. That's 16 files; the shared helpers, the UI globals and the registration stay (4.2k lines). **Step 3, first slice ✅** An audit of the unit methods retail's UI calls, traced and read. The one that changed the sim, `SetCustomName`, now goes through the command stream as Moho's does. `IsInCategory` takes a category's name, as UserUnit's does.<br><br>**Step 3 ✅** The UI's units are Moho's `UserUnit`: `moho.user_unit_methods`, its 36 methods.<br>• They read the tick's `WorldSnapshot`. In a drawn game that is the renderer's capture; otherwise the UI captures the tick the first time it asks. The snapshot now carries the unit settings and economy they report.<br>• They change the unit only through the command stream (`ProcessInfo`, `SetCustomName`).<br>• A UI script no longer sees the sim's methods (`Kill`, `SetHealth` and the rest), and between ticks it sees a unit as the last tick left it.<br>• *Left:* `GetStat` and `CanAttackTarget` read the live unit, read-only, since the snapshot carries no stats or weapons. No retail UI script calls them.<br><br>**Step 4 ✅** A fresh UI Lua state for every game and front end, as Moho starts each (design: `docs/plans/2026-09-25-ui-state-per-game-design.md`):<br>• A launch and a return to the lobby tear the old state down and build a new one; the old state's controls get their `OnDestroy` first.<br>• `FrontEndData` keeps deep copies in a state of its own, so the lobby's session config reaches the game.<br>• A second game in a run had broken: retail's `unitviewDetail` reads back a module global it set to nil. It works now, and `data.load_flow` loads from the game menu, returns to the lobby and loads again, with no Lua error. |
| M192 | Split the executable | Design: `docs/plans/2026-09-24-m192-split-executable-design.md`. **Step 1 ✅** The game no longer ships its tests. `main()` is `osc::app::run` in the `osc_app` library (`src/app/`). The ~130 test modes, `integration_tests.cpp` and the LAN harnesses are the integration runner, `osc_integration` (`tests/integration/runner/`), which CTest runs. It reaches the run through `TestModes` hooks placed where the modes stood. The format ratchet now follows moved files.<br><br>**Step 2a ✅** `app.cpp` (2.9k lines) is split into `cli.cpp`, `ui_globals.cpp`, `session.cpp`, `reload.cpp` and `frames.cpp`, with a private `app_internal.hpp`, and keeps only `run()`. It is a pure move: a token count finds only 23 `static`s dropped and the new declarations, and a four-AI game's 3,000-tick checksum trace is byte-identical before and after.<br><br>**Step 2b ✅** `run()` is an `App` object that owns what the run builds. `run()` reads the command line once, into `Options`. `App`'s phases are its methods: `boot()` (`boot.cpp`), `run_window()` (`window.cpp`) and `run_headless()` (`headless.cpp`). The tests' `Engine` is a view of `App`. It is a scripted move: the token diff shows only the moved declarations and the method frames, and the checksum trace is byte-identical.<br>*Next:* break the windowed loop's frame into handlers (step 2c). |
| M193 | Unit order handlers | ✅ `Unit::update` (1,251 lines) is five named phases, and its 22 order kinds are one handler each in `src/sim/unit_orders.cpp`, returning `Next`, `Hold` or `Gone`. Converted by a script that tracks block nesting; four-AI games on four seeds (6,000 ticks, and 18,000 on one) give byte-identical per-domain checksum traces before and after. Design: `docs/plans/2026-09-24-m193-unit-order-handlers-design.md`. |
...
| M206 | Order fidelity | Blueprint ranges for build, reclaim and repair. Teleport timing. Nukes and tactical missiles via silo weapons. Ferry. Design: `docs/plans/2026-09-24-m206-order-fidelity-design.md`. **M206a ✅** Silo weapons:<br>• Units build missiles for their counted weapons, ordered (`IssueSiloBuildNuke`/`Tactical`, the orders panel) or in auto mode, for the projectile's cost at their build rate, assisted by guarding engineers.<br>• Launch orders fire the right weapon through retail's script, at a unit or the ground, and wait for a missile; a mobile launcher moves into range.<br>• A silo's missile leaves along its muzzle (a nuke rises before it turns), and weapons can hold ground targets.<br><br>**M206b ✅** Missile defence:<br>• Projectiles answer category tests (their blueprint's categories, `ALLPROJECTILES`), so retail's collision rules, flares and depth charges work; `ALLUNITS` excludes them.<br>• Weapons with `TargetType = 'RULEWTT_Projectile'` target enemy missiles and torpedoes, at most `DesiredShooterCap` at a time (one anti-nuke per nuke), leading them.<br>• A weapon's `ProjectileLifetime` sets its shots' lifetime.<br><br>**M206c ✅** Beam weapons:<br>• A collision beam reaches from its muzzle along its facing, stops at the first thing its weapon may hit, and hits every `CollisionCheckInterval + 1` ticks through `OnImpact`; beam weapons fire through their scripts.<br>• Bones are placed at the blueprint's `UniformScale`: every muzzle had been about 14 times too far from its unit.<br>• A blast reaches what its radius touches, measured to shapes, so a beam or shell on a big unit's hull hurts it.<br>• *Left:* a mobile launcher too close to its target drops the order (M206e); torpedoes from surfaced subs don't dive and dived subs sink only while moving (a naval slice).<br><br>**M206d ✅** Charged orders:<br>• Economy events draw their cost through their army's economy, slow with a stall, and report progress to their scripts, so teleports and weapon recharges cost what retail says.<br>• OverCharge fires the OverCharge weapon through its script, which swaps out the main gun and draws 5000 energy.<br>• A teleport holds its queue until the warp; called off, the unit hears `OnFailedTeleport`.<br><br>**M206e ✅** Ranges and the queue, from Moho's decompiled unit tasks ([faf-re](https://github.com/Draiget/faf-re)):<br>• **Ranges:** build, repair, reclaim and capture measure a gap: the distance between centres less the worker's footprint and the target's skirt or footprint. It is compared with `MaxBuildDistance` (default 5), or a fixed 5 and 10 for capture. Out of reach, a unit walks just clear of its target first.<br>• **Footprints:** a missing one rounds the blueprint's size up, as Moho does, and props have one.<br>• **Guards:** they help only within reach of the work, and only if they repair or reclaim.<br>• **The queue:** `Issue*` appends, as Moho's does.<br>• **Launch orders:** a launcher too close backs off; with no missile, the order asks the silo for one.<br><br>**M206f ✅** Ferries:<br>• A transport's `Ferry` orders are a route that stays queued. It makes a beacon at the first point (its `AI.BeaconName`), shared by transports ordered together.<br>• Units ordered to load at the beacon wait there. The ferry carries them along the route's waypoints to the last point, unloads, and flies back for more.<br>• A beacon goes with the last route that holds it.<br><br>**M206g ✅** Specific unloads: `IssueTransportUnloadSpecific(transports, category, position)` picks the cargo in the category when it is given, and the transport drops only those units there; with none aboard, no order is given (Moho's `UNITCOMMAND_TransportUnloadSpecificUnits`). The chosen cargo travels with the order: replays (format v6) and lockstep frames carry it, and the orders checksum domain hashes it.<br><br>**M206h ✅** Factory assist, from Moho's guard task (`TryDispatchFactoryOrUpgradeFromGuardQueues`):<br>• A factory guarding a factory takes work from its queue. Each time it is free, the first build order it can build leaves the guarded queue (repeating, it goes to the back) and it builds the unit itself. The one the other factory is building is left alone, even where Moho's rule for a repeating assister would take it: that rule relies on order counts and repeat queues, which aren't here yet.<br>• If the guard order goes mid-build, the unit it was building is dropped, as a factory's is when its build order goes.<br>• `unit:CanBuild` is the sim's check now (`blueprint_can_build`).<br>• *Left:* its own queued builds first, and upgrades.<br><br>**M206i ✅** Repeat queues, from Moho's command dispatch:<br>• A factory repeating its queue sends each finished build order to the back and starts the next one the following tick. An order for several units goes back one unit at a time, which builds them in Moho's order.<br>• A build that fails still leaves the queue.<br>• *Left:* while it goes round, the queue shows a split order as two, where Moho shows one with its count.<br><br>**M206j ✅** Factory rally orders (Moho's factory command queue):<br>• A factory keeps rally orders apart from its build orders. Each unit it finishes takes them after its roll-off move, and a unit built for a guarded factory takes that factory's.<br>• With none, a factory is given its blueprint's initial rally, 5 ahead of it by default. `GetRallyPoint` returns the first, which retail's roll-off reads.<br>• `IssueFactoryRallyPoint` adds one, and `IssueClearFactoryCommands` clears them. It had been clearing the builds retail's AI queued before setting its rally, so the AI's factories now keep building, and their units gather at its rally markers.<br><br>**M206k ✅** Players' rally orders, as Moho's UI gives them:<br>• A move, patrol or transport call splits the selection. Its RALLYPOINT units (factories) take it as a factory command, a flag the order carries through lockstep frames and replays (format v7). The sim puts it in their rally orders, replacing them unless it is queued, and leaves their builds alone.<br>• A right-click on an enemy no longer gives a factory an Attack order, which had cleared its builds.<br>• A selected factory's rally orders are drawn with its orders.<br>• A patrol whose points a unit already stands on no longer spins within one tick: the next leg starts next tick. A path that starts and ends in the same cell no longer spends the tick's path budget, so units parked on their patrol point can't stall everyone else's.<br><br>**M206l ✅** Transports carry by their attach points, as Moho's `CAiTransportImpl` does. Design: `docs/plans/2026-09-25-m206l-transport-slots-design.md`.<br>• A transport's bones are sorted by name into class lists. A medium or large unit also holds the 2 or 4 small bones nearest its point. So a UEF T2 transport takes 14 small units, or 6 medium, or 3 large, and mixed loads share the small bones.<br>• Retail transports have no `Class1Capacity`, which the engine had read, so each had carried any number of units of any size.<br>• A carried unit hangs from its bone, by its own `AttachPoint` bone or its centre, facing as the bone does. Scripts hear the real bone's name. Unloaded units are set down where they hung, not on top of each other.<br>• A slot is freed when its unit is unloaded, destroyed aboard or taken off.<br>• The slots are in the checksum.<br>• *Left:* the pickup and the drop (M206m/n).<br><br>**M206m ✅** The pickup, as Moho's shared load order (`CUnitLoadUnits` and `CUnitCallTransport`):<br>• Units told to load onto a transport call it. It gives slots to them largest first, and a unit without one is left, its order done (`OnTransportFull`).<br>• The transport flies to the centre of those it took and comes down to its `TransportHoverHeight`. It waits there up to 300 ticks.<br>• Each unit walks under its bone and beams up over 10 ticks, easing from the ground to the bone, then attaches (`OnStartTransportBeamUp` / `OnStopTransportBeamUp`).<br>• A pickup stopped on the way is aborted (`OnTransportAborted`). Its units' orders end, and they don't call it back.<br>• *Left:* the drop (M206n).<br><br>**M206n ✅** The drop, as Moho's `CUnitUnloadUnits`:<br>• Over its drop point, a transport comes down to its hover height before setting its cargo down. The unload order now flies: it had dragged aircraft along the ground.<br>• Each unit is set down on the ground where it hung, only if its footprint fits there (Moho's `FitsAt`). Over deep water, nothing is dropped.<br>• Idle, a transport hovers low with cargo aboard and climbs back to its flying height without.<br>• A ferry drops its cargo the same way.<br><br>**M206o ✅** Naval depth, from Moho's decompiled `CUnitMotion` and `Projectile`:<br>• A sub dives and surfaces where it stands, as well as on the move. Each tick it goes `DiveSurfaceSpeed`/10 along a sine, down to its `Physics.Elevation` (no deeper than a quarter over the seabed).<br>• Its layer changes when it gets there. Its script hears `OnMotionVertEventChange`: Down, then Bottom; Up, then Top.<br>• A torpedo aims a quarter under the surface, and is held under it only once in the water. So a surfaced sub's torpedoes dive in (`OnEnterWater`), run under, and reach dived subs. A torpedo dropped from above falls.<br><br>**M206p ✅** A factory guarding a factory builds its own queued units and upgrades first, as Moho's guard task dispatches its owner's queue before the guarded one's. The first of them runs ahead of the guard order, which carries on once it is done. **M206q ✅** Carriers keep the aircraft they build in storage (`Transport.StorageSlots`, apart from the cargo slots) and launch them when told to unload, from their launch bones: the next tick, then one every 2 ticks (Moho's task timing). Stored units go with the carrier. Every retail carrier had stayed busy for good after its first build, since its script asks `TransportHasAvailableStorage`. Aircraft landing on a carrier, and storage moving with a given carrier, are still to come. **M206r ✅** Air staging, from Moho's `CUnitRefuel` and `ProcessFuelLevels`. Design: `docs/plans/2026-09-26-m206r-air-staging-design.md`.<br>• The orders panel's Dock (`IssueDockCommand`) sends the selection's aircraft to the nearest idle air staging platform. When it can't take them all, they are shared among the platforms within 100 of it, roomiest first.<br>• Retail's AI refuels the same way, with `IssueTransportLoad` onto a platform.<br>• An aircraft takes a slot of its class, flies over the bone, settles onto it facing the bone's way, and docks.<br>• Docked, it refuels at `FuelRechargeRate / FuelUseTime / 10` times the platform's `RefuelingMultiplier` a tick. It repairs at the platform's `RefuelingRepairAmount` a second while its army pays `RepairConsume*` (`OnStartRefueling`).<br>• Full and repaired, it is let go at the platform's next 9-tick look and climbs back to its flying height.<br>• Unloading a platform (the AI's release) sends its aircraft from where they sit to the point. A platform that dies lets them go in the air.<br>• Fuel now burns only in flight. `OnRunOutOfFuel` and `OnGotFuel` fire as the tank reaches and leaves 0.<br>• *Left:* landing on carriers, patrol auto-refuel, the no-platform voice-overs. **M206s ✅** Aircraft land on carriers, from Moho's `CUnitCarrierLand` and `CUnitCarrierRetrieve`. Design: `docs/plans/2026-09-26-m206s-carrier-landing-design.md`.<br>• A carrier reserves storage places round robin over its landing points. Each pass round makes the next aircraft wait 3 ticks more, and a reserved place counts as taken.<br>• Aircraft ordered aboard (`IssueTransportLoad` onto the carrier) come in from 20 back along their place's facing, wait their turn and are stored. The carrier takes its own share of the order: it stops (or surfaces) and waits for them (`OnStartTransportLoading` / `OnStopTransportLoading`).<br>• Docked at a carrier (the Dock button now includes carriers), an aircraft lands and refuels and repairs stored. Full, it is launched from the next launch bone and climbs back to its flying height.<br>• A launched aircraft flies on from the height it left at: it had kept a stale height.<br>• *Left:* a carrier holding its aircraft's height as it moves, patrol auto-refuel.<br>• **Aircraft over water** hold their height over the water's surface, not the seabed: Moho's `CUnitMotion` samples max(terrain, water) unless `Air.FlyInWater`. Over water deeper than their `Elevation`, they had flown under the waves. **M206u ✅** A plain right-click gives each selected unit its default order, as FA does (per its manual's "Giving Orders"):<br>• On an enemy, Attack; Capture for a unit that can capture but not attack.<br>• On an ally, Assist (Guard); Repair for an engineer on one under construction; a load for a unit a transport can carry; Dock for an aircraft at a staging platform.<br>• On a wreck or prop, Reclaim for an engineer.<br>• Units that can't take the order, and every unit on open ground, move there. A mixed selection splits into one order per kind. It had only moved or attacked.<br>• Repairing a unit under construction builds it, as Moho's repair task does: alongside its builder, or alone to completion when it was left unfinished. A unit completes once (`OnStopBeingBuilt`), however many build it.<br>• *Left:* right-clicks on ferry points and on waypoints (patrol, coordinated attack), and a transport right-clicking the units it would carry. |
| M207 | AI query fidelity | Threat maps with visibility, water ratio, blocking terrain, PBM build locations. **M207a ✅** `brain:GetMapWaterRatio` measures the map as Moho does: heightfield points every 8 units, the border left out, under the water's surface (SCMP_009 is 59% water). It had answered 0, so the AI took every map for land. `FormPlatoon` already matches Moho's: it neither sets `PlatoonHandle` nor calls `OnUnitsAddedToPlatoon`, and it returns a platoon short of a squad's minimum. An earlier note here said otherwise.<br><br>**M207b ✅** Influence maps, from Moho's `CInfluenceMap`. Design: `docs/plans/2026-09-25-m207b-influence-map-design.md`.<br>• Each army keeps a grid of threat: cells of max(32, map size / 16), usually 16 × 16.<br>• Every army-count ticks, what its intel detects is reported. That covers every enemy unit it has in sight, radar, sonar or omni, allies' units, and structures it has once seen. It updates every 30 ticks.<br>• A mobile unit out of sight keeps full threat for 10 updates, then loses 0.02 per update. A structure stays until the army sees it gone.<br>• Threat is split, as Moho's is, into land, air, naval, structures, commander, experimental and the Anti lanes. The Anti lanes count only units the army has seen closely.<br>• The brain's threat queries answer from it in cells. `AssignThreatAtPosition` works, and `FindPlaceToBuild` passes over sites with too much AntiSurface threat.<br>• The AI no longer sees through fog.<br><br>**M207c ✅** `brain:CheckBlockingTerrain(start, end, arc)`, from Moho's decompiled `CAiBrain` and `CHeightField` ([faf-re](https://github.com/Draiget/faf-re)). It had always answered "clear".<br>• A straight shot (`'none'`) is one segment. A `'low'` or high arc is four chords of a half-sine whose peak is a quarter of the span times 0.5 or 2.<br>• The start is lifted 1 and the end 0.5.<br>• Each chord is cast against the heightfield as Moho casts it: each cell is two triangles, split along its diagonal.<br>• Retail's naval `CheckNavalPathing` passes an end it never set, which reads as the map's origin, as Moho's does.<br>• PBM build locations needed nothing: retail's `aibrain.lua` defines every `PBM*` method in Lua, over the engine's.<br>• *Left:* jammers' false blips (no jammer makes them yet). |
| M208 | Save/load | Design: `docs/plans/2026-09-25-m208-save-load-design.md`. A save is the game's recording (setup and commands), plus the orders still to run. **M208a done:** loading replays the recording to the saved tick, checked against its checksum trail, then hands over to the player. That is correct by construction, since replays play back identically. Retail's `InternalSaveGame` and `LoadSavedGame` are bound, and the load catches up in the game's frame loop. `data.save_load` and `data.load_flow` hold it. **M208b ✅ measured:** a load costs what the game took to play. A 30-minute four-AI game loads in 6 minutes and a 60-minute one in about 40 (Release, before M224's sim speedups). So loads speed up with the sim (M224), and long games need M208c: state snapshots (Pluto for Lua 5.0 plus C++ serializers), a milestone of its own. Loading from inside a game works since M191 step 4 (a fresh UI state per game) and #94 (the relaunch renderer fixes). |
...
| M224 | Sim hot paths | Incremental visibility, spatial target acquisition, path caching and reuse, O(1) thread wake, per-tick GC stepping instead of a full collect every 50 ticks. Late four-AI games (15,000+ ticks, 1,700–1,800 units) are where the sim is slow, so they drive the order. Loading a saved game replays it, so a faster sim also means faster loads (M208b). **M224a ✅** Category expressions are compiled once per query, and units carry their categories as interned ids. Before, every AI query walked the Lua table and hashed strings for each unit: a third of late-game time. The traces are byte-identical, and the 18,000-tick sim runs 21% faster. **M224b ✅** The AI's radius queries (27% of late-game time after M224a) read a grid of units alone. It holds the entities themselves, so props aren't walked, ids aren't looked up, and fewer ids are sorted. Measured side by side, the 18,000-tick sim takes 450 s on main, 337 s after M224a and 256 s after M224b (−43%), with identical traces. **M224c ✅** Entity lookup is an index (ids only grow), and timed effects cache their `LIFETIME`. The registry's hash probe was paid once per attached effect per tick, and a string hash once per timed effect per tick. Another 6–7% on the 18,000-tick game. **M224d ✅** Emitters end. An emitter's effect ends after its `Lifetime` in ticks, rounded up (a muzzle flash's 0.1 emits for one tick), or a script's `LIFETIME`, as Moho's `CEfxEmitter::ProcessLifetime` has it; only a negative Lifetime emits on. `CreateEmitterOnEntity` attaches its emitter, so a projectile's trail goes with the shot. Before, every muzzle flash, explosion and trail stayed in the registry for the rest of the game: 388,000 effects by tick 16,000, walked three times a tick (a third of late-game time). The renderer ends emitters by the same rule. `Repeattime` is only the curve period, not a pause between bursts, which had made most one-shots repeat. Side by side, the 18,000-tick game runs in 108 s against 147 s on main (−27%), with the same units at every tick and the golden replays unchanged. **M224e ✅** The sim's Lua state is collected every 70 ticks, as in Moho's `Sim::AdvanceBeat`, not every 50. Its heap is mostly live, so every collection is the forced one, a 60–90 ms mark late in the game. Measured on top of the fix that makes threads and effects hold their Lua handles (retail's weak trash bags had lost them at each collection, so the schedule had changed the game). The game now matches at every logged tick, and the 18,000-tick run takes 85.8 s against 88.8 s. **M224f ✅** The AI's placement queries (`CanBuildStructureAt`, 6.7% of late-game time) gather the army's pending build sites only once a site passes the terrain checks. They read each blueprint's placement rules once per game. Hot per-unit category tests (the influence map's, the snapshot's icons) test interned ids, not hashed names. The game is unchanged at every logged tick; 68.7 s against 72.9 s. |
