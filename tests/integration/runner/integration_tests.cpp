...
void test_prebuilt(TestContext& ctx) {
    ...
    auto r = ctx.lua_state.do_string("InitializePrebuiltUnits('ARMY_1')");
    ...
    ctx.lua_state.do_string(
        "__osc_off = table.getn(__osc_inits); ScenarioInfo.Options.PrebuiltUnits = 'On'");
    session.spawn_prebuilt_units(L, ctx.sim);
    ctx.lua_state.do_string(R"(
